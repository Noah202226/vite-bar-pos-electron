...
export default function FloorPlan() {
  const { activeOrder, loadTableOrder } = useOrderStore()
  const selectedTable = activeOrder?.tableNumber

  const handleTableClick = (num) => {
    loadTableOrder(num.toString())
  }
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-6">
            <TableButton
              ...
              onClick={() => handleTableClick('V1')}
            />
            <TableButton
              label="V2"
              isVIP
              isSelected={selectedTable === 'V2'}
              onClick={() => handleTableClick('V2')}
            />
            {[1, 2, 3, 4, 5, 6, 7, 8].map((num) => (
              <TableButton
                key={num}
                label={num}
                isSelected={selectedTable === num.toString()}
...
function TableButton({ label, isVIP, isSelected, onClick }) {
  return (
    <button
      onClick={onClick}
      className={`h-28 rounded-4xl border-2 flex flex-col items-center justify-center transition-all ${isSelected ? 'bg-indigo-600 border-indigo-500 text-white shadow-2xl -translate-y-2' : isVIP ? 'bg-slate-900 border-amber-500/30 text-amber-500 hover:border-amber-500' : 'bg-slate-900 border-slate-800 text-slate-500 hover:border-slate-600'}`}
    >
...
function PromoCard({ title, desc, time, icon, color }) {
  return (
    <div
      className={`min-w-60 p-5 rounded-3xl border bg-linear-to-br flex flex-col justify-between ${color}`}
    >
